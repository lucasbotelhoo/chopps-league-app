...
def init_data():
    if not os.path.exists(FILE_PARTIDAS):
        df = pd.DataFrame(columns=["Data", "Time 1", "Time 2", "Placar Time 1", "Placar Time 2", "Local"])
...
def tela_presenca_login():
    ...
    if not st.session_state.usuario_logado:
        ...
        if st.button("Sair"):
            st.session_state.usuario_logado = None
            st.session_state.pagina = "🏠 Tela Principal"
            st.experimental_rerun()
...
# Controle programático da página
...
# Navegação principal
if st.session_state.pagina == "🏠 Tela Principal":
    tela_principal(partidas)
elif st.session_state.pagina == "📊 Estatísticas da Partida":
    partidas = tela_partida(partidas)
elif st.session_state.pagina == "👟 Estatísticas dos Jogadores":
    jogadores = tela_jogadores(jogadores)
elif st.session_state.pagina == "🎲 Sorteio de Times":
    tela_sorteio()
elif st.session_state.pagina == "✅ Presença e Login":
    tela_presenca_login()
